...
class PreprocessingConfig(BaseModel):
    ...
    exponential: bool = Field(
        default=True,
        description="Apply exponential function to enhance contrast"
    )
    ...
    gblur: int = Field(
        ...
        ge=0,
        le=50,
        description="Gaussian blur sigma (0 means no blur)"
    )
    ...
    def validate_conflicting_options(self):
        ...
        if self.exponential and self.logarizer:
            raise ValueError("Cannot apply both exponential and logarithmic transformations")
        return self
...
class BlobDetectionConfig(BaseModel):
    ...
    min_blob_size: PositiveFloat = Field(
        default=5.0,
        le=1000.0,
        description="Minimum blob size to detect (pixels)"
    )
    
    max_blob_size: PositiveFloat = Field(
        default=20.0,
        le=2000.0,
        description="Maximum blob size to detect (pixels)"
    )
    
    blob_step: PositiveFloat = Field(
        default=1.0,
        le=10.0,
        description="Step size between consecutive blob scales"
    )
    
    downscale: PositiveFloat = Field(
        default=4.0,
        le=20.0,
        description="Image downscaling factor before detection"
    )
    ...
    def validate_max_blob_size(cls, v: float, info) -> float:
        ...
        if hasattr(info, 'data') and 'min_blob_size' in info.data:
            ...
            if v <= min_size:
                raise ValueError(f"max_blob_size ({v}) must be > min_blob_size ({min_size})")
        return v
...
class FileProcessingConfig(BaseModel):
    ...
    blob_downscale: PositiveFloat = Field(
        default=7.0,
        le=50.0,
        description="Downscaling factor for blob detection"
    )
    
    target_memory_gb: PositiveFloat = Field(
        default=4.0,
        le=128.0,
        description="Target GPU memory usage in GB"
    )
    
    stream_large_files: bool = Field(
        default=True,
        description="Whether to use streaming for large files"
    )
...
class MRCMetadata(BaseModel):
    ...
    mode: int = Field(
        ge=0,
        le=6,
        description="MRC data type mode (0=int8, 1=int16, 2=float32, etc.)"
    )
    ...
    def validate_mean_in_range(cls, v: float, info) -> float:
        ...
        if hasattr(info, 'data') and 'data_min' in info.data and 'data_max' in info.data:
            data_min = info.data['data_min']
            data_max = info.data['data_max']
            if not (data_min <= v <= data_max):
                raise ValueError(f"data_mean ({v}) must be between data_min ({data_min}) and data_max ({data_max})")
        return v
...
class AdaptiveFilterConfig(BaseModel):
    ...
    initial_slope: PositiveFloat = Field(
        default=10.0,
        le=100.0,
        description="Initial slope for sigmoid thresholding"
    )
    ...
    @classmethod
    def validate_kernel_size(cls, v: Union[int, Tuple[int, int]]) -> Union[int, Tuple[int, int]]:
        ...
        if isinstance(v, int):
            ...
            if len(v) != 2:
                raise ValueError(f"Kernel size tuple must have 2 elements, got {len(v)}")
...
class ValidationPipeline(BaseModel):
    ...
    blob_detection: BlobDetectionConfig = Field(
        default_factory=BlobDetectionConfig,
        description="Blob detection configuration"
    )
    
    file_processing: Optional[FileProcessingConfig] = Field(
        default=None,
        description="File processing configuration (for batch operations)"
    )
    
    adaptive_filtering: Optional[AdaptiveFilterConfig] = Field(
        default=None,
        description="Adaptive filtering configuration"
    )
    
    def validate_for_single_image(self) -> Tuple[PreprocessingConfig, BlobDetectionConfig]:
        """
    ...
    def validate_for_batch_processing(self) -> Tuple[PreprocessingConfig, BlobDetectionConfig, FileProcessingConfig]:
        """
        ...
        if self.file_processing is None:
            raise ValueError("file_processing configuration is required for batch processing")
        
        return self.preprocessing, self.blob_detection, self.file_processing
    
    def validate_for_adaptive_processing(self) -> Tuple[PreprocessingConfig, AdaptiveFilterConfig]:
        """
        ...
        if self.adaptive_filtering is None:
            raise ValueError("adaptive_filtering configuration is required for adaptive processing")
